...
class SourceCaptureUiContractTests(unittest.TestCase):
    ...
    def test_manual_capture_uses_a_hand_icon_and_an_actual_hover_or_focus_tooltip(self):
        ...
        self.assertIn('padding: 12px 16px', SOURCE)
        self.assertIn('font-family: var(--font-body)', SOURCE)
        self.assertIn('font-size: 14px', SOURCE)
        self.assertIn('border-top: 7px solid var(--graphite)', SOURCE)
        self.assertIn('Tooltip 对话气泡允许使用 `6px` 圆角', DESIGN)
