...
- 竞品档案卡片固定为 `360px × 360px` 正方形，并在档案轨道内横向排列；竞品过多时只允许轨道自身左右滚动，不得拉伸卡片或造成整页横向溢出。卡片内容超出固定高度时在卡片内部纵向滚动。
- 横向对比的对比维度列固定 `160px` 并持续吸附在矩阵左侧；竞品列固定 `280px`，竞品过多时只允许矩阵容器自身左右滚动。固定列使用 Medium 分割线，不使用阴影。
