...
- **一级业务区块标题：** “竞品档案”“竞品横向对比”“洞察结论”等同级模块必须复用同一中文字体、`20px / 900 / 1.25`、工业编号和 Strong 分割线；不得为单个新增模块另设字号、Display 字体或独立间距。
...
- “清空示例”复用白底黑框 Secondary，作为页面内操作紧邻橙色“添加竞品”；不得移到预览页外置工具栏或其他区域。仅当前 Tab 被标记为首次系统示例且仍有示例内容时显示，清空后立即隐藏。
...
- 只有首次打开时创建的默认 Tab 可以带系统示例标记；用户手动新增的 Tab 必须保持空白且不得继承该标记。示例标记按 Tab 隔离，切换 Tab 时同步决定“清空示例”的可见性。
...
- 洞察新增与编辑以“证据”为唯一可选择的关联主体；竞品与分析维度由已选证据的 `competitorId` 与 `dimensionIds[]` 自动求并集并只读回显，不允许形成三组互不一致的并列选择。每条候选证据使用 `40px × 40px` 纯眼睛 Icon 作为“查看原证据”入口并复用完整证据详情弹窗；Icon 固定在证据行右侧操作列并与该行垂直居中，各行操作位必须右对齐且不得挤动证据文字；所有设备默认隐藏，仅在当前证据行 Hover 或 `focus-within` 时显示，不得因粗指针或触屏能力而全部常驻。Icon 自身 Hover/Focus 复用 Secondary 的黑底白色 Icon 反转反馈，但不得产生位置偏移。
- `competitorIds[]` 与 `dimensionIds[]` 仅是证据关联的派生缓存，不是可独立信任的输入。加载、迁移、导入和最终持久化时必须先清理无效或重复的 `evidenceIds[]`，再按有效证据重新求并集；证据的竞品、维度或存续状态变化后，所有受影响洞察必须立即执行同一重算。
- “洞察结论 / INSIGHT FINDINGS”的中文、英文、工业编号、字体、字号、基线、间距和分割线必须与“竞品横向对比 / COMPARISON MATRIX”复用同一级业务区块 Header 规则。
...
- 删除竞品、分析维度或证据时保留洞察；先更新或移除受影响证据引用，再根据剩余有效证据重新派生竞品与维度。删除洞察不得改变其他研究数据。
