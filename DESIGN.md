...
- **内容语义图画：** 一级区块标题左侧使用统一的 `60px × 42px` 微型工业构图：浅灰坐标轴、黑色分层结构板、白色留孔与橙色倾斜板件构成空间层次。档案＝分层资料卡、对比＝对齐矩阵、洞察＝信号路径；不得使用星球、圆盘、轨道等天体意象，也不得以通用方形 Icon 容器、流程数字、斜杠或文本替代。窄屏统一缩小为 `48px × 34px`，装饰图画必须 `aria-hidden="true"`。
...
- 竞品、证据、矩阵维度、矩阵单元格的“查看全部”、洞察、Tab 与图片的编辑/删除属于上下文操作：桌面端默认隐藏，仅在对应父信息区 Hover 或 `focus-within` 时显示；键盘聚焦按钮自身时也必须可见。触屏或粗指针设备第一次点按信息区后显示，第二次点按原内容继续原操作；不得默认常驻，也不得变成触屏不可达。
