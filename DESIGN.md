...
- **内容语义图画：** 一级区块标题左侧使用统一的 `60px × 42px` 微型工业构图：浅灰坐标轴、细轨道、黑色圆盘与白色留孔构成空间层次，橙色倾斜板件承载具体业务符号。档案＝分层资料卡、对比＝对齐矩阵、洞察＝信号路径；不得以通用方形 Icon 容器、流程数字、斜杠或文本替代。窄屏统一缩小为 `48px × 34px`，装饰图画必须 `aria-hidden="true"`。
